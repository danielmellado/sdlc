...
_scan_cves_quay_correlate() {
    ...
    # --- Rebase check: count fixable CVEs and find newer base image tags ---
    local fixable_count
    fixable_count="$(echo "$response" | jq "[.data.Layer.Features[] | ${rpm_filter} | .Vulnerabilities[]? | select(.FixedBy != null and .FixedBy != \"\")] | length")"
    local unfixable_count=$(( base_total - fixable_count ))

    if [[ "$base_total" -gt 0 ]]; then
        echo ""
        echo "=== BASE IMAGE REBASE RECOMMENDATION ==="
        echo ""
        printf "  %-35s %s\n" "Fixable by rebase:" "$fixable_count of $base_total CVEs"
        printf "  %-35s %s\n" "No upstream fix yet:" "$unfixable_count CVEs"

        # Try to detect base image from labels
        local base_ref=""
        local image_labels
        image_labels="$(skopeo inspect "docker://${image}" 2>/dev/null | jq -r '.Labels // {}')"
        base_ref="$(echo "$image_labels" | jq -r '
            .["io.openshift.build.source-image"] //
            .["org.opencontainers.image.base.name"] //
            empty' 2>/dev/null)"

        if [[ -n "$base_ref" ]]; then
            local base_repo="${base_ref%%:*}"
            local current_tag="${base_ref##*:}"
            echo ""
            echo "  Base image: $base_ref"

            local latest_tags
            latest_tags="$(skopeo list-tags "docker://${base_repo}" 2>/dev/null | jq -r '.Tags[]' | sort -V | tail -5)"
            if [[ -n "$latest_tags" ]]; then
                local newest
                newest="$(echo "$latest_tags" | tail -1)"
                echo "  Latest tags available:"
                echo "$latest_tags" | sed 's/^/    /'
                if [[ "$newest" != "$current_tag" ]]; then
                    echo ""
                    echo "  ⬆  Rebase to ${base_repo}:${newest} to pick up fixes for $fixable_count CVEs."
                else
                    echo ""
                    echo "  ℹ  Already on latest tag. Unfixed CVEs need upstream package updates."
                fi
            fi
        else
            echo ""
            echo "  ℹ  Could not detect base image reference from labels."
            echo "     Check manually: skopeo list-tags docker://<base-image>"
        fi

        if [[ "$fixable_count" -eq "$base_total" && "$app_total" -eq 0 ]]; then
            echo ""
            echo "  🎯 A simple rebase would eliminate ALL known CVEs."
        fi
    fi
}
