...
### Step 8c: Base Image Rebase Check (with --correlate)

When `--correlate` is active and base image CVEs have `FixedBy` versions,
check whether a newer base image tag exists that includes those fixes.

1. **Identify the base image**: inspect the scanned image's labels for base
   image metadata:
   ```bash
   skopeo inspect docker://<IMAGE> | jq -r '
     .Labels["io.openshift.build.source-image"] //
     .Labels["org.opencontainers.image.base.name"] //
     .Labels["io.buildah.version"] // empty'
   ```
   For Red Hat images, also check `com.redhat.component` and the `release`
   label — the base is typically `ubi8-micro`, `ubi9-minimal`, or similar.

2. **List newer tags**: use skopeo to find available tags:
   ```bash
   skopeo list-tags docker://<BASE_IMAGE_REGISTRY>/<BASE_IMAGE_REPO> | \
     jq -r '.Tags[]' | sort -V | tail -10
   ```

3. **Count fixable CVEs**: from the base image CVE bucket, count how many
   have a non-empty `FixedBy` field — these would be resolved by a rebase.

4. **Report**:
   ```
   ### Base Image Rebase Recommendation
   Base image: <base_image>:<current_tag>
   Latest available: <base_image>:<latest_tag>
   Fixable by rebase: N of M base image CVEs have fixes available
   Unfixable (no upstream fix): K CVEs

   Recommendation: Rebase to <base_image>:<latest_tag> to resolve N CVEs.
   ```
   If ALL base image CVEs are fixable and application CVEs are zero,
   emphasize: "A simple rebase would eliminate all known CVEs."
